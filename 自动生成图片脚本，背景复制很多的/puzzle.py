...
def resize_pic(in_name,size,sel):

    img = Image.open(in_name)
    if sel == 'samll':
        img = ImageOps.fit(img, (size, size), Image.ANTIALIAS)
    if sel == 'big':
        img = ImageOps.fit(img, (img.size[0], img.size[1]), Image.ANTIALIAS)
    return img

def convert_image(path):
    img = resize_pic(path,SLICE_SIZE,'samll')
...
if __name__ == '__main__':
    ...
    img = resize_pic(image,OUT_SIZE,'big')
